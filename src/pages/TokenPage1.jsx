...
const TokenPage = () => {
  ...
  const handleFetchOrderBook = async () => {
    try {
      const data = await fetchOrderBook(cryptoA, cryptoB);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black text-white flex flex-col items-center py-12 px-4 md:px-8 space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center pb-3 border-b border-gray-700">
                ...
                <span className={`font-semibold ${tokenData._24hr_change >= 0 ? "text-green-500" : "text-red-500"}`}>
      ...
      <div className="w-full bg-gray-800/60 backdrop-blur-lg rounded-2xl shadow-2xl border border-gray-700/50 p-6">
        ...
        <div className="flex flex-col space-y-4">
          ...
          <select value={orderMode} onChange={(e) => setOrderMode(e.target.value)} className="p-3 rounded bg-gray-700">
          ...
          <button onClick={placeOrder} className="bg-yellow-500 p-3 rounded text-black font-bold hover:bg-yellow-600">
            Place Order
          </button>
        </div>
      </div>
          ...
          <tbody>
            {orderBook.map((order) => (
              <tr key={order._id} className="border-b border-gray-700">
                <td className={order.type === "buy" ? "text-green-400" : "text-red-400"}>{order.type}</td>
                <td>{order.amount}</td>
