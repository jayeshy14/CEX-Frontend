...
const WithdrawPage = () => {
  ...
  const [isCryptoWithdraw, setIsCryptoWithdraw] = useState(true);
  const [selectedCrypto, setSelectedCrypto] = useState(null);
  const [selectedChain, setSelectedChain] = useState("");

  useEffect(() => {
    // Fetch cryptocurrencies from the server
  ...
  const handleWithdraw = async () => {
    if (parseFloat(amount) > usdBalance) return alert("Insufficient balance");
