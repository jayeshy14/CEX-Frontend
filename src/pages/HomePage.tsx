...
const HomePage = () => {
  const [cryptocurrencies, setCryptocurrencies] = useState<CoinGeckoCrypto[]>([]);
  ...
  useEffect(() => {
    const fetchCryptos = async () => {
      try {
        ...
        const data = (await response.json()) as CoinGeckoCrypto[];
  ...
  return (
    <main className="flex-grow p-8 mx-auto bg-gray-900 text-white min-h-screen">
      ...
      <section className="bg-gray-900 shadow-lg rounded-2xl p-10 mb-12 space-y-8">
        <h2 className="text-5xl font-semibold text-yellow-400 mb-6 text-center">
